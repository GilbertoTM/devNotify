...
function App() {
  const { user, isAuthenticated, isLoading, login, signUp, logout, getAllUsers } = useAuth();
  const [activeView, setActiveView] = useState<'notifications' | 'projects'>('notifications');
  ...
  const handleAddIntegration = (projectId: string) => {
    setSelectedProjectForIntegration(projectId);
